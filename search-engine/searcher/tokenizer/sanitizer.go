...
// compiledPatterns holds every pattern from sanitizingPatterns, compiled exactly
// once at package init. Sanitize is called for every word of every search
// query, so recompiling a regexp per word (the previous behavior) meant
// re-paying regex-compilation cost on every request.
var compiledPatterns = func() map[string]*regexp.Regexp {
compiled := make(map[string]*regexp.Regexp, len(sanitizingPatterns))
	for lang, pattern := range sanitizingPatterns {
		compiled[lang] = regexp.MustCompile(pattern)
	}
	return compiled
}()
...
func Sanitize(input []string, language string) []string {
	...
	re, ok := compiledPatterns[language]
	if !ok {
		re = compiledPatterns["default"]
	}
	for _, word := range input {
		item := re.ReplaceAllString(word, "")
