...
// compiledPatterns pre-compiles every language's regex exactly once, at
// package load time.
var compiledPatterns = func() map[string]*regexp.Regexp {
	compiled := make(map[string]*regexp.Regexp, len(sanitizingPatterns))
	for language, pattern := range sanitizingPatterns {
		compiled[language] = regexp.MustCompile(pattern)
	}
	return compiled
}()
